...
class COCODataset(Dataset):
    ...
    def __getitem__(self, idx: int) -> Any:
        img, heatmaps, heat_mask, pafs, paf_mask = self.pull_item(idx)
    ...
    def pull_item(self, idx: int) -> Any:
        ...
        meta_data_idx = self.df.iloc[idx]["meta_data_idx"]
...
def data_test():
    ...
    json_path = "dataset/COCO.json"

    train_dataset = COCODataset(train_path, json_path, phase="train", transform=DataTransform())
    val_dataset = COCODataset(val_path, json_path, phase="val", transform=DataTransform())
    ...
    imgs, heatmaps, heat_mask, pafs, paf_mask = next(batch_iterator)
